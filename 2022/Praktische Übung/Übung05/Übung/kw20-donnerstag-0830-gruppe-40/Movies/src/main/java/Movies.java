public class Movies {
  ...
  public static void nameSort(Movie[] array) {
    ...
    for (a = 0; a < highSub; ++a) {
      for (b = 0; b < highSub-1; ++b) {
        String first = array[b].getName(); // wahrscheinlich sollte hier a nicht b sein
        String second = array[b + 1].getName();
        if (first.compareTo(second) > 0) {
          temp = array[b];
          array[b] = array[b+1]; // wahrscheinlich sollte hier a nicht b sein
          array[b + 1] = temp;
        }
      }
    }
